...
- 🔧 19 built-in tools: `read_file` (offset/limit/tail) / `read_files` (batch reads) / `compare_files` (text diff) / `project_stats` (project file metrics) / `file_info` (single-file metadata) / `replace_in_files` (glob-based batch replacement) / `copy_file` (safe text-file copy) / `move_file` (safe move/rename) / `delete_file` (safe single-file deletion) / `write_file` / `edit_file` (newline-style tolerant: LF↔CRLF normalized matching) / `list_directory` / `glob` / `grep` (multiline cross-line matching) / `run_command` / `bash` / `powershell` / `stop` (command tools auto-pick Git Bash / PowerShell); `edit_file` / `write_file` show a colored diff preview before executing
