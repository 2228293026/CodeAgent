...
- 📊 Usage visibility: status bar shows per-turn tokens, current context size ctx (with percentage; window auto-detected for common models) and thinking effort (`auto` probes supported levels and picks the highest); `/compact` compresses history manually (with progress; ESC cancels)
