...
public sealed class ReadFileTool : ITool
{
    ...
    public async Task<string> ExecuteAsync(JsonObject? args, AgentContext ctx, CancellationToken ct)
    {
        ...
        if (showStats)
        {
            ...
            var words = SkipDirs.CountWords(text, ct);
            ...
            try
            {
                ...
                foreach (var d in OrderEntries(directories, sortBy, reverse))
                {
                    ...
                    if (!filesOnly)
                    {
                        ...
                        if (skipEmptyDirs && !SkipDirs.EnumerateFilesPruned(d, ct: ct, followSymlinks: false)
                        ...
                        if (showFileCount)
                        {
                            ...
                            var fileInDir = SkipDirs.EnumerateFilesPruned(d, recursive ? int.MaxValue : 0, ct: ct, followSymlinks: false)
