...
/// <summary>批量复制多个文本文件，复用 copy_file 的安全检查并汇总每个映射的结果。</summary>
public sealed class CopyFilesTool : ITool
{
    private const int DefaultMaxFiles = 100;
    private const int MaxFiles = 500;
    private readonly CopyFileTool _copier = new();

    public string Name => "copy_files";
    public string Description =>
        "批量复制多个文本文件。每个映射包含 source 和 destination，可单独设置 overwrite；支持全局 dry_run、错误继续和最大映射数。";
    ...
    public JsonObject Parameters { get; } = new()
    {
        ...
        ["properties"] = new JsonObject
        {
            ["mappings"] = new JsonObject
            {
                ...
                ["minItems"] = 1,
                ["description"] = "复制映射数组，例如 [{\"source\":\"a.txt\",\"destination\":\"backup/a.txt\"}]",
            },
            ["dry_run"] = new JsonObject { ["type"] = "boolean", ["description"] = "只预览全部复制，不写盘（默认 false）" },
    ...
    public async Task<string> ExecuteAsync(JsonObject? args, AgentContext ctx, CancellationToken ct)
    {
        ...
        for (var i = 0; i < Math.Min(mappings.Count, maxFiles); i++)
        {
            ...
            try
            {
                ...
                var result = await _copier.ExecuteAsync(one, ctx, ct);
                success++;
            ...
            catch (Exception ex) when (ex is ToolException or IOException or UnauthorizedAccessException or ArgumentException or InvalidDataException)
            {
                failed++;
                if (stopOnError)
                    throw new ToolException($"映射[{i}] 复制失败: {ex.Message}");
                Append(output, $"映射[{i}] 失败: {ex.Message}");
            }
        }

        var header = $"批量复制: 成功 {success} 个，失败 {failed} 个" +
