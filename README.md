...
REPL 命令：`/help` `/clear` `/compact` `/cls` `/model [名称|编号]` `/provider [名]` `/config` `/session` `/setup` `/undo` `/diff` `/save` `/load` `/resume [编号]` `/export` `/copy` `/stats` `/retry` `/tools` `/providers` `/models [关键字]` `/history [N]` `/thinking` `/mode` `/access` `/diag` `/exit` `/quit`。
