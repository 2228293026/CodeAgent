...
| `obsolete_usage_report` | 只读检查 [Obsolete]：标注了却仍在用、error:true 仍在用、未写替代方案 |
| `extension_method_report` | 只读检查扩展方法：非 static 类里的扩展方法、首参缺 this、定义在嵌套类 |
| `record_mutability_report` | 只读检查 record 不变性：持有可变集合字段、有会改状态的成员、record struct |
| `nullable_annotation_report` | 只读检查可空性标注：#nullable disable、裸引用类型、集合元素未标 ? |
| `gitignore_quality_report` | 只读检查 .gitignore：反斜杠跨平台失效、绝对路径、通配符过宽、常见产物漏忽略 |
| `todo_comment_report` | 只读检查 TODO/FIXME 标记：无归属人、无关联工单、标记在被注释掉的代码里 |
| `csharp_keyword_report` | 只读检查 C# 关键字：声明名与关键字同形或仅差大小写、多余的 @ 前缀 |
| `large_method_report` | 只读检查超大方法：方法体过长、嵌套过深、参数过多、分支过多 |
| `duplicate_method_report` | 只读检查重复实现：方法体逐字重复、方法名重复、重复注释块 |
| `config_key_usage_report` | 只读检查配置项：声明了却从未读取、读取了却无声明、命名风格不一致 |
| `magic_number_report` | 只读检查魔法数字：重复字面量、比较/算术里的裸字面量、端口/超时/缓冲区量级 |
| `gitattributes_quality_report` | 只读检查 .gitattributes：缺 text=auto、缺 eol 声明、规则矛盾/重复、二进制误当文本 |
| `line_ending_mixed_report` | 只读检查换行符混杂：仓库 CRLF/LF 混用、单文件内混用、末尾无换行 |
| `stale_comment_report` | 只读检查脱节注释：复述标识符的注释、注释掉的死代码、待办/过期标记 |
| `string_concat_perf_report` | 只读检查字符串拼接性能：循环体内反复 += 或 = a + b、循环里不变的字面量前缀、超长多段拼接 |
| `test_placeholder_report` | 只读检查测试占位与空壳：没有断言的测试、恒真断言、空断言、被注释掉的测试 |
| `public_api_surface_report` | 只读检查公开 API 稳定性：可变默认值、公开字段、只能靠位置传的 bool 参数 |
| `optional_param_dropped_report` | 只读检查接口实现与接口声明的签名漂移：实现漏了可选参数默认值、少了 ref/out/in、返回类型不一致 |
| `switch_exhaustiveness_report` | 只读检查 switch 完备性：缺 default、对 enum 的 switch 漏成员、case 重复书写 |
| `linq_query_report` | 只读检查 LINQ 查询陷阱：循环体内重复查询、同一序列枚举两遍、循环体里的单谓词 LINQ |
| `swallowed_exception_report` | 只读检查被吞掉的异常：空 catch 块、既不抛也不记录、catch 类型与抛出类型不符 |
| `equals_gethashcode_report` | 只读检查 Equals/GetHashCode 契约：漏写 GetHashCode、漏写 Equals、两个方法依据的字段不一致 |
| `deferred_query_report` | 只读检查 LINQ 延迟执行陷阱：未物化的查询、foreach 中修改集合、同一变量被反复枚举 |
| `boxing_allocation_report` | 只读检查热路径上的隐式分配：装箱、循环体内的字符串插值、ToList() 后立刻被枚举 |
| `time_unit_confusion_report` | 只读检查时间单位的陷阱：名字说毫秒值像秒、名字说秒值像毫秒、超时值没有单位 |
| `culture_format_report` | 只读检查区域设置陷阱：ToString/Parse 未指定文化、ToUpper/ToLower 未用不变文化 |
| `concurrent_mutation_report` | 只读检查并发写入共享状态：并行写共享集合、未加锁的计数器自增、并行里改写捕获变量 |
| `undisposed_stream_report` | 只读检查流与句柄泄漏：可释放对象没包 using、手动 Dispose、直接返回可释放对象 |
| `null_assignment_report` | 只读检查可空性陷阱：非空变量被赋 null、判空后仍直接解引用、非空方法 return null |
| `boolean_parameter_report` | 只读检查布尔参数陷阱：可选参数排在必选参数之后、语义不明的布尔开关、签名里参数重名 |
| `hardcoded_secret_report` | 只读检查源码里的硬编码凭据：API Key/Token/口令字面量、连接串口令、密钥进了日志或异常 |
| `path_escape_report` | 只读检查路径逃逸：Path.Combine 未校验根目录、`..` 上跳、用字符串前缀判断路径归属 |
| `duplicate_logic_report` | 只读检查复制粘贴：同一字面量多个常量名、除名字外相同的方法体、散落的魔法数字 |
| `silent_catch_report` | 只读检查静默失败：空 catch、catch 变量未使用、catch 后返回默认值 |
| `interface_contract_report` | 只读检查接口契约漂移：接口成员未实现、参数类型悄悄对不上、可能漏写 override |
| `testability_report` | 只读检查可测性：过长的方法、private 里藏着的长逻辑、if/else 不成对 |
| `lock_misuse_report` | 只读检查锁的使用：lock 块里有 await、锁了值类型/字面量、同一把锁嵌套 |
| `dependency_inversion_report` | 只读检查依赖倒置缺口：字段/构造参数声明成具体类型、直接 new 基础设施 |
| `ownership_report` | 只读检查资源所有权：IDisposable 却没 Dispose、收到可释放参数但没说清所有权、注入字段释放责任不明 |
| `weak_assertion_report` | 只读检查弱断言：测试里没有断言、恒真断言、测试内吞掉异常、共享可变静态状态 |
| `untrusted_input_report` | 只读检查不可信输入：命令/SQL 拼接、未校验的外部路径、动态加载外部程序集 |
| `retry_safety_report` | 只读检查重试的正确性：固定间隔无退避、没有次数上限、重试里包含非幂等写操作 |
| `state_machine_report` | 只读检查状态机漏洞：状态字段被赋枚举外的值、switch 缺 default、状态变更无前置校验 |
| `observability_gap_report` | 只读检查可观测性缺口：catch 无痕迹、关键写操作无日志、日志无关联标识 |
| `time_zone_report` | 只读检查时区陷阱：Now 与 UtcNow 混用、Kind 丢失后换算、用 DateTime 存时刻 |
| `collection_edge_report` | 只读检查集合边界：未判空就取首/聚合、索引访问代替 TryGetValue、循环里的 List.Contains |
| `contract_evolution_report` | 只读检查契约演进风险：必需参数没有默认值、返回内部集合引用、接口成员无默认实现 |
| `config_robustness_report` | 只读检查配置脆弱性：环境变量不判空、解析失败静默兜底、数值型配置没有范围校验 |
| `float_precision_report` | 只读检查浮点/金额精度：浮点字面量参与算术、浮点用 == 比较、Math.Round 未指定舍入模式、Parse 未固定区域 |
| `enum_flag_report` | 只读检查枚举/标志误用：枚举与数字字面量比较、[Flags] 枚举用 == 判断、布尔与 true/false 比较 |
| `async_misuse_report` | 只读检查 async/await 误用：.Result/.Wait() 同步阻塞死锁、async 里没有 await、发射后不管导致异常丢失 |
| `api_version_pin_report` | 只读检查项目文件依赖版本：版本用 ^ ~ * 范围未锁死、预发布版本、同一个包锁了多个版本 |
| `nested_try_report` | 只读检查 try/finally 控制流陷阱：内层 catch 吞掉异常、finally 里 return/throw 覆盖原异常、循环里 finally 用 return 结束迭代 |
| `iterator_yield_report` | 只读检查迭代器陷阱：迭代器里用 return 提前截断序列、yield 出现在 try-catch 中、迭代器里做阻塞 I/O |
| `switch_fallthrough_report` | 只读检查 switch 穿透：case 分支没写 break/return 就落进下一个 case、default 不在最后 |
| `linq_deferred_report` | 只读检查 LINQ 延迟执行：延迟查询被反复使用、循环里反复枚举同一条查询、查询后改写集合 |
| `string_interpolation_report` | 只读检查字符串拼接写法：循环里 + 拼接（O(n²)）、多段拼接未用插值、循环里拼日志 |
| `array_bounds_report` | 只读检查数组越界：arr[arr.Length] off-by-one、减法索引无下界保护、可能为空的结果直接取下标 |
| `readonly_struct_report` | 只读检查 struct 语义：readonly struct 里声明 setter、可变 struct 当集合键后又被改写、防御性拷贝 |
| `integer_overflow_report` | 只读检查整数运算静默错误：unchecked 溢出回绕、整数除法向零截断、窄化强制转换丢精度 |
| `default_parameter_report` | 只读检查默认参数值陷阱：默认值是共享的可变对象、默认取 DateTime.Now、默认值不是常量表达式 |
| `lost_update_report` | 只读检查共享计数器的丢失更新：共享字段 ++/+=/读改写却既没 lock 也没 Interlocked |
| `doc_comment_report` | 只读检查 XML 文档注释：公开成员缺 summary、标签未闭合、summary 为空 |
| `partial_method_report` | 只读检查 partial 成员：partial 只写一半、方法只有声明没有实现、类型跨文件拆分 |
| `obsolete_usage_report` | 只读检查 [Obsolete]：标注了却仍在用、error:true 仍在用、未写替代方案 |
| `extension_method_report` | 只读检查扩展方法：非 static 类里的扩展方法、首参缺 this、定义在嵌套类 |
...
│   ├── ObsoleteUsageReportTool.cs         # obsolete_usage_report Obsolete 使用报告
│   ├── ExtensionMethodReportTool.cs       # extension_method_report 扩展方法报告
│   ├── RecordMutabilityReportTool.cs      # record_mutability_report record 可变性报告
│   ├── NullableAnnotationReportTool.cs    # nullable_annotation_report 可空性标注报告
│   ├── GitignoreQualityReportTool.cs      # gitignore_quality_report .gitignore 质量报告
│   ├── TodoCommentReportTool.cs           # todo_comment_report TODO 标记报告
│   ├── CsharpKeywordReportTool.cs         # csharp_keyword_report C# 关键字报告
│   ├── LargeMethodReportTool.cs           # large_method_report 超大方法报告
│   ├── DuplicateMethodReportTool.cs       # duplicate_method_report 重复实现报告
│   ├── ConfigKeyUsageReportTool.cs        # config_key_usage_report 配置项使用报告
│   ├── MagicNumberReportTool.cs           # magic_number_report 魔法数字报告
│   ├── GitattributesQualityReportTool.cs  # gitattributes_quality_report .gitattributes 质量报告
│   ├── LineEndingMixedReportTool.cs        # line_ending_mixed_report 换行符混杂报告
│   ├── StaleCommentReportTool.cs           # stale_comment_report 脱节注释报告
│   ├── StringConcatPerfReportTool.cs       # string_concat_perf_report 字符串拼接性能报告
│   ├── TestPlaceholderReportTool.cs        # test_placeholder_report 测试占位报告
│   ├── PublicApiSurfaceReportTool.cs       # public_api_surface_report 公开 API 稳定性报告
│   ├── OptionalParamDroppedReportTool.cs   # optional_param_dropped_report 接口签名漂移报告
│   ├── SwitchExhaustivenessReportTool.cs   # switch_exhaustiveness_report switch 完备性报告
│   ├── LinqQueryReportTool.cs               # linq_query_report LINQ 查询陷阱报告
│   ├── SwallowedExceptionReportTool.cs     # swallowed_exception_report 异常吞噬报告
│   ├── EqualsGetHashCodeReportTool.cs      # equals_gethashcode_report Equals/GetHashCode 契约报告
│   ├── DeferredQueryReportTool.cs         # deferred_query_report LINQ 延迟执行报告
│   ├── BoxingAllocationReportTool.cs      # boxing_allocation_report 隐式分配报告
│   ├── TimeUnitConfusionReportTool.cs     # time_unit_confusion_report 时间单位混淆报告
│   ├── CultureFormatReportTool.cs         # culture_format_report 区域设置报告
│   ├── ConcurrentMutationReportTool.cs    # concurrent_mutation_report 并发写入报告
│   ├── UndisposedStreamReportTool.cs      # undisposed_stream_report 流泄漏报告
│   ├── NullAssignmentReportTool.cs        # null_assignment_report 可空性报告
│   ├── BooleanParameterReportTool.cs      # boolean_parameter_report 布尔参数报告
│   ├── HardcodedSecretReportTool.cs        # hardcoded_secret_report 硬编码凭据报告
│   ├── PathEscapeReportTool.cs             # path_escape_report 路径逃逸报告
│   ├── DuplicateLogicReportTool.cs         # duplicate_logic_report 重复逻辑报告
│   ├── SilentCatchReportTool.cs            # silent_catch_report 静默失败报告
│   ├── InterfaceContractReportTool.cs      # interface_contract_report 接口契约报告
│   ├── TestabilityReportTool.cs            # testability_report 可测性报告
│   ├── LockMisuseReportTool.cs             # lock_misuse_report 锁使用报告
│   ├── DependencyInversionReportTool.cs    # dependency_inversion_report 依赖倒置报告
│   ├── OwnershipReportTool.cs              # ownership_report 资源所有权报告
│   ├── WeakAssertionReportTool.cs          # weak_assertion_report 弱断言报告
│   ├── UntrustedInputReportTool.cs        # untrusted_input_report 不可信输入报告
│   ├── RetrySafetyReportTool.cs           # retry_safety_report 重试安全报告
│   ├── StateMachineReportTool.cs          # state_machine_report 状态机报告
│   ├── ObservabilityGapReportTool.cs      # observability_gap_report 可观测性缺口报告
│   ├── TimeZoneReportTool.cs              # time_zone_report 时区报告
│   ├── CollectionEdgeReportTool.cs        # collection_edge_report 集合边界报告
│   ├── ContractEvolutionReportTool.cs     # contract_evolution_report 契约演进报告
│   ├── ConfigRobustnessReportTool.cs      # config_robustness_report 配置健壮性报告
│   ├── FloatPrecisionReportTool.cs         # float_precision_report 浮点精度报告
│   ├── EnumFlagReportTool.cs              # enum_flag_report 枚举/标志报告
│   ├── AsyncMisuseReportTool.cs           # async_misuse_report async 误用报告
│   ├── ApiVersionPinReportTool.cs         # api_version_pin_report 依赖版本报告
│   ├── NestedTryReportTool.cs            # nested_try_report 嵌套 try 报告
│   ├── IteratorYieldReportTool.cs         # iterator_yield_report 迭代器报告
│   ├── SwitchFallthroughReportTool.cs     # switch_fallthrough_report switch 穿透报告
│   ├── LinqDeferredReportTool.cs          # linq_deferred_report LINQ 延迟执行报告
│   ├── StringInterpolationReportTool.cs   # string_interpolation_report 字符串拼接报告
│   ├── ArrayBoundsReportTool.cs           # array_bounds_report 数组越界报告
│   ├── ReadonlyStructReportTool.cs        # readonly_struct_report readonly struct 报告
│   ├── IntegerOverflowReportTool.cs       # integer_overflow_report 整数溢出报告
│   ├── DefaultParameterReportTool.cs      # default_parameter_report 默认参数报告
│   ├── LostUpdateReportTool.cs            # lost_update_report 丢失更新报告
│   ├── DocCommentReportTool.cs           # doc_comment_report 文档注释报告
│   ├── PartialMethodReportTool.cs         # partial_method_report partial 成员报告
│   ├── ObsoleteUsageReportTool.cs         # obsolete_usage_report Obsolete 使用报告
│   ├── ExtensionMethodReportTool.cs       # extension_method_report 扩展方法报告
