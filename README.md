...
交互输入：输入 `/`（兼容中文输入法的全角 `／`）自动弹出**命令菜单**（全部命令 + 说明，**默认不选中任何项**，`↑`/`↓` 选择后回车才执行、`Esc` 关闭、继续输入即过滤，无匹配时显示提示）；**快捷键**：`Alt+M`（或 `Ctrl+Shift+M`）模式菜单、`Alt+U` 撤销、`Alt+D` 查看 diff、`Alt+N` 新建会话、`Ctrl+L` 清屏（部分终端吞 Alt 时用 Ctrl+Shift 组合）；无菜单时 `↑`/`↓` 浏览命令历史（持久化在 `.codeagent/history.txt`）。每轮提示符上方显示**状态栏**：`⏵ 模式 · 模型 · 目录 · token 用量`。提示符显示当前模式、模型与目录，如 `[debug|laguna-s-2.1:free] CodeAgent>`。
