...
REPL 命令：`/help` `/clear` `/compact` `/cls` `/model [名称]` `/config` `/session` `/setup` `/undo` `/diff` `/save` `/load` `/resume [编号]` `/export` `/stats` `/retry` `/tools` `/providers` `/models` `/history` `/thinking` `/mode` `/access` `/diag` `/exit` `/quit`。
