...
交互输入：输入 `/` 自动弹出**命令菜单**（全部命令 + 说明），`↑`/`↓` 选择、回车执行、`Esc` 关闭、继续输入即过滤；无菜单时 `↑`/`↓` 浏览命令历史（持久化在 `.codeagent/history.txt`）；`Ctrl+L` 或 `/cls` 清屏。提示符显示当前模式、模型与目录，如 `[debug|laguna-s-2.1:free] CodeAgent>`。
