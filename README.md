...
- 🔧 内置 10 个工具：`read_file`（offset/limit/tail）/ `write_file` / `edit_file`（换行风格容错：LF↔CRLF 自动归一化匹配）/ `list_directory` / `glob` / `grep`（支持 multiline 跨行匹配）/ `run_command` / `bash` / `powershell` / `stop`（命令类工具自动选用 Git Bash / PowerShell）；`edit_file` / `write_file` 执行前展示彩色 diff 预览
...
REPL 命令：`/help` `/clear` `/compact` `/cls` `/model [名称|编号]` `/provider [名]` `/config` `/session` `/setup` `/undo` `/diff` `/save` `/load` `/resume [编号]` `/find <关键字>` `/export` `/copy` `/prompt` `/files` `/stats` `/retry` `/tools` `/providers` `/models [关键字]` `/history [N]` `/thinking` `/shell` `/mode` `/access` `/diag` `/exit` `/quit`。
