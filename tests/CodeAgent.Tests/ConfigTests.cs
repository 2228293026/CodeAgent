...
public class ConfigTests : IDisposable
{
    ...
    [Fact]
    public void Load_AutoCompactPercent_DefaultsToFifty()
    {
        // 未配置时默认 50%，避免加载历史时每次截断都触发压缩
        var path = Path.Combine(_dir, "empty.json");
        File.WriteAllText(path, "{}");
        Assert.Equal(0, AgentConfig.Load(path).AutoCompactPercent);
    }

    [Fact]
    public void Load_MaxToolIterations_DefaultsToZero()
    {
        // 未配置时默认 0（表示不限制工具调用轮数）
        var path = Path.Combine(_dir, "empty3.json");
        File.WriteAllText(path, "{}");
        Assert.Equal(0, AgentConfig.Load(path).MaxToolIterations);
    }
    ...
    [Fact]
    public void Load_ExportDir_DefaultsToCodeagentExports()
    {
        // 未配置时默认导出到 .codeagent/exports（历史/导出统一目录）
        var path = Path.Combine(_dir, "empty16.json");
        File.WriteAllText(path, "{}");
        Assert.Equal(".codeagent/exports", AgentConfig.Load(path).ExportDir);
    }
    ...
    [Fact]
    public void Load_AllowCommands_DefaultsToTrue()
    {
        // 未配置时默认允许命令执行（run_command/bash/powershell 可用）
        var path = Path.Combine(_dir, "empty13.json");
        File.WriteAllText(path, "{}");
        Assert.True(AgentConfig.Load(path).AllowCommands);
    }
