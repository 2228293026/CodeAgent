...
public class ToolRegistryEdgeTests
{
    ...
    public void CreateDefault_RegistersExpectedToolSet()
    {
        ...
        Assert.Equal(
            new[] { "apply_patch", "bash", "compare_files", "copy_file", "copy_files", "delete_file", "edit_file", "file_info", "find_duplicates", "git_status", "glob", "grep", "list_directory", "move_file", "powershell", "project_stats", "read_file", "read_files", "replace_in_files", "run_command", "session_search", "stop", "write_file" },
            names);
    }
}
