...
public class ToolRegistryTests : IDisposable
{
    ...
    [Fact]
    public void CreateDefault_RegistersOneHundredFortyFiveTools()
    {
        ...
        var expected = new[]
        {
            ...
            "session_search", "stop", "apply_patch", "async_correctness_report", "string_performance_report", "null_safety_report", "logging_practice_report", "api_usage_report", "api_compatibility_report", "data_contract_report", "performance_anti_pattern_report", "test_quality_report", "resource_lifecycle_report", "localization_readiness_report", "documentation_sync_report", "input_sanitization_report", "file_safety_report", "dependency_version_report", "error_handling_detail_report", "collection_performance_report", "concurrency_risk_report", "dead_implementation_report", "duplicate_code_report", "test_isolation_report", "input_validation_report",
        };
        foreach (var name in expected)
            Assert.True(names.Contains(name), $"缺少工具: {name}");
        Assert.Equal(145, expected.Length); // 不应有多余工具
