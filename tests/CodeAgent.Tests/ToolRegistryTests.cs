...
public class ToolRegistryTests : IDisposable
{
    ...
    [Fact]
    public void CreateDefault_RegistersOneHundredTwentyNineTools()
    {
        ...
        var expected = new[]
        {
            ...
            "session_search", "stop", "apply_patch", "async_correctness_report", "collection_performance_report", "concurrency_risk_report", "dead_implementation_report", "duplicate_code_report", "test_isolation_report", "input_validation_report",
        };
        foreach (var name in expected)
            Assert.True(names.Contains(name), $"缺少工具: {name}");
        Assert.Equal(129, expected.Length); // 不应有多余工具
