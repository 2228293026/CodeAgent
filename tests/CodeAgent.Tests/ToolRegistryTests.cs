...
public class ToolRegistryTests : IDisposable
{
    ...
    [Fact]
    public void CreateDefault_RegistersOneHundredTwentyFiveTools()
    {
        ...
        var expected = new[]
        {
            ...
            "session_search", "stop", "apply_patch", "concurrency_risk_report", "dead_implementation_report", "input_validation_report",
        };
        foreach (var name in expected)
            Assert.True(names.Contains(name), $"缺少工具: {name}");
        Assert.Equal(125, expected.Length); // 不应有多余工具
