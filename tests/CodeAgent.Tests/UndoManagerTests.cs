...
public class UndoManagerTests : IDisposable
{
    ...
    [Fact]
    public void TryUndo_EditLargeFileFallback_ReplacesOnlyOneOccurrence()
    {
        var path = Path.Combine(_dir, "duplicate.txt");
        ...
        Assert.NotNull(desc);
        Assert.Equal("old middle new", File.ReadAllText(path));
    }
    ...
    [Fact]
    public void SnapshotDir_DoesNotFollowSymlinkOutsideWorkspace()
    {
        var outside = Path.Combine(Path.GetDirectoryName(_dir)!, "codeagent-undo-outside-" + Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            Assert.DoesNotContain("outside-link/secret.txt", snapshot.Texts.Keys);
        }
        finally
        {
